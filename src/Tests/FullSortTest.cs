...
public class FullSortTest
{
    ...
    {
       // Directory.Delete(_sortOptions.SortedChunksPath, true);
    }

    [Test]
    public async Task InMemorySortTest()
    {
        var sortExecutor = _serviceProvider.GetRequiredService<SortExecutor>();
        
        _watch.Start();
        await sortExecutor.RunSort();
        _watch.Stop();
        Console.WriteLine($"Sort time: {_watch.Elapsed.TotalSeconds} sec.");
